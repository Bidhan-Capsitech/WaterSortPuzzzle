...
public class IAPManager : MonoBehaviour
{
    ...
    void LockAllBacks()
    {
        for (int i = 0; i < m_general_settings.shopBackgrounds.Length; i++)
        {
            m_general_settings.shopBackgrounds[i].isOpened = false;
        }
        m_general_settings.shopBackgrounds[0].isOpened = true;
        PlayerPrefs.SetInt("all_BackgroundsPurchased", 0);
    }
    ...
    void LockAllBottles()
    {
        for (int i = 0; i < m_general_settings.shopTubes.Length; i++)
        {
            m_general_settings.shopTubes[i].isOpened = false;
        }
        m_general_settings.shopTubes[0].isOpened = true;
        PlayerPrefs.SetInt("all_tubesPurchased", 0);
