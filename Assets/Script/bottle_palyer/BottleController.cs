...
public class BottleController : MonoBehaviour
{
    ...
    public void setupBottleTransferSpeed()
    {
        ...
        bottleRotateSpeed = 40 * speedValue * Time.deltaTime;
    }
    ...
    public void playNot_mus_anime()
    {
        ...
        ///////Vibration add and bottle vibration effect////////
        // bottle_Animator.Play("Base Layer.bottle_animation",0);
        //  Invoke("desable_anime",1.0f);
        //Handheld.Vibrate();
        StartCoroutine(ReturnAndShake());

    }

    IEnumerator BottleShakeSmooth()
    {
        // use world position (transform.position) so it matches ReturnAndShake / Move coroutines
        Vector3 originalPos = transform.position;

        AnimationCurve curve = new AnimationCurve(
            new Keyframe(0, 0),
            new Keyframe(0.25f, 0.063f),
            new Keyframe(0.5f, -0.063f),
            new Keyframe(0.75f, 0.063f),
            new Keyframe(1f, 0)
        );
        ...
        {
            float xOffset = curve.Evaluate(elapsed / duration);
        ...
        // ensure exact original alignment
        transform.position = originalPos;
    }

    IEnumerator ReturnAndShake()
    {
        // Step 1: smoothly move bottle back to original position
        Vector3 startPos = transform.position;
        float t = 0f;

        while (t < 1f)
        {
            t += Time.deltaTime * 5f; // move speed
            transform.position = Vector3.Lerp(startPos, originalPosition, t);
            yield return null;
        }

        // Ensure bottle fully reaches original position
        transform.position = originalPosition;

        // Step 2: vibrate & shake at that position
        Handheld.Vibrate(); // phone vibration
        yield return StartCoroutine(BottleShakeSmooth());
    }
    ...
    IEnumerator RotateBottle()
    {
        ...
        while (t < timeToTotate)
        {
            ...
            if (fillamounts[numberOfColorsInBottle] > (FillAmountCurve.Evaluate(angleValue) +0.005f))
            {
                ...
                float abc = FillAmountCurve.Evaluate(lastAngleValue) - FillAmountCurve.Evaluate(angleValue);
                bottleControllerRef.FillUp(abc);
            }
...
        lineRenderer.enabled = false;
    ...
    void is_ref_full()
    {
        ...
        if (testMuchColors() == true && t_1 == true && numberOfColorsInBottle == 4)
        {
            ...
            // instentate the vfx
            Vector3 spawnPos = transform.position + Vector3.down * 0.34f;
            fullVFX = Instantiate(bottleFull_Effects, spawnPos, Quaternion.identity); 
